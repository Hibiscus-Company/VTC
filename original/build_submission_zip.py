...
def main():
    ...
    p.add_argument("--max_mb", type=float, default=350.0,
               help="cap in MiB. The competition cap is 350 MiB = 367,001,600 bytes (confirmed: a 357 MB-decimal zip was accepted). Budgeting in decimal MB wastes ~17 MB.")
    ...
    def total_mb():
        return sum(len(b) for i in scenes.values() for b in i["blobs"].values()) / (1024 * 1024)
    ...
    while improved:
        ...
        for s in sorted(scenes, key=lambda s: sum(len(b) for b in scenes[s]["blobs"].values())):
            ...
            if total_mb() + (new_b - cur_b) / (1024 * 1024) <= args.max_mb - margin:
