...
def main():
    ...
    p.add_argument("--ckpt_every", type=int, default=0,
                   help="save a rolling ckpt_latest.pt every N steps (0=off). Same schema as "
                        "the final ckpt.pt so render_gsplat.py loads it directly; atomic write "
                        "(tmp + os.replace). For unattended/overnight runs. NOTE: mip3d bake "
                        "is final-save only, so a mip3d run's rolling ckpt renders un-baked.")
    ...
    if args.init_ckpt:
        ...
        RGB2SH = lambda rgb: (rgb - 0.5) / 0.28209479177387814  # inlined from utils/sh_utils.py
    ...
    for step in range(args.iters):
        ...
        # Rolling crash-safe checkpoint for unattended (overnight) runs: same schema as the
        # final ckpt.pt so render_gsplat.py can load it directly. Atomic write so a kill
        # mid-save never leaves a corrupt file. Uses EMA params when EMA is active, matching
        # what the final save would ship at this point in training.
        if args.ckpt_every > 0 and step > 0 and step % args.ckpt_every == 0:
            roll = (ema_params if ema_params is not None
                    else {n: params[n].detach() for n in params.keys()})
            os.makedirs(args.out, exist_ok=True)
            _tmp = os.path.join(args.out, "ckpt_latest.pt.tmp")
            torch.save({"splats": {n: roll[n].detach().cpu() for n in roll.keys()},
                        "sh_degree": args.sh_degree, "K": K_np, "wh": (W, H),
                        "ut": args.ut, "k1": k1, "eps2d": args.eps2d, "step": step},
                       _tmp)
            os.replace(_tmp, os.path.join(args.out, "ckpt_latest.pt"))
