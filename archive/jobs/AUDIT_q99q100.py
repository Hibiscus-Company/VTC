"""AUDIT: isolate q99ss2 vs q100ss2 (the r33 HCM0421 change) on real public test GT."""
...
ARMS=[("q100ss2",dict(quality=100,subsampling=2,optimize=True,progressive=True)),
      ("q99ss2", dict(quality=99, subsampling=2,optimize=True,progressive=True))]
...
for TAG in TOWERS:
    ...
    acc={a[0]:[0.,0.,0.] for a in ARMS}; per={a[0]:[] for a in ARMS}
    selfmse={a[0]:[] for a in ARMS}; bts={a[0]:0 for a in ARMS}; t0=time.time()
    for s in stems:
        ...
        for nm,kw in ARMS:
            b=io.BytesIO(); Image.fromarray(u8).save(b,"JPEG",**kw); bts[nm]+=len(b.getvalue())
            j8=np.asarray(Image.open(io.BytesIO(b.getvalue())).convert("RGB"))
            selfmse[nm].append(float(((j8.astype(np.float64)-u8.astype(np.float64))**2).mean()))
            r=torch.from_numpy(np.ascontiguousarray(j8.astype(np.float32)/255.)).permute(2,0,1).unsqueeze(0).to(dev)
            with torch.no_grad():
    ...
    for nm,_ in ARMS:
        ...
        rows[TAG][nm]=(100*(0.4*(1-L)+0.3*S+0.3*min(P/50.,1.)),d.mean(),d.std(ddof=1)/np.sqrt(N),int((d>0).sum()),N,P,S,L,
                       float(np.mean(selfmse[nm])),bts[nm]/N/2**20)
    print(f"  {TAG} n={N} {time.time()-t0:.0f}s",flush=True)
print("\nq99ss2 vs q100ss2 -- single member + shipped field, REAL public test GT")
print(f"{'scene':>9} {'arm':>9} {'SCORE':>9} {'d_vs_q100':>10} {'se':>7} {'wins':>8} {'dPSNR':>8} {'dSSIM':>9} {'dLPIPS':>9} {'selfRMSE':>9} {'MiB/img':>8}")
for TAG in rows:
    b=rows[TAG]["q100ss2"]
    for nm,_ in ARMS:
        sc,dm,se,w,N,P,S,L,sm,mb=rows[TAG][nm]
        print(f"{TAG:>9} {nm:>9} {sc:9.4f} {dm:+10.4f} {se:7.4f} {w:3d}/{N:<4d} {P-b[5]:+8.4f} {S-b[6]:+9.5f} {L-b[7]:+9.5f} {np.sqrt(sm):9.4f} {mb:8.3f}")
ds=[rows[t]["q99ss2"][1] for t in rows]
ses=[rows[t]["q99ss2"][2] for t in rows]
print(f"\nq99ss2 5-scene mean {np.mean(ds):+.4f}  sd {np.std(ds,ddof=1):.4f}  se_of_mean {np.std(ds,ddof=1)/np.sqrt(len(ds)):.4f}"
      f"  per-scene {[round(x,4) for x in ds]}  positive {sum(1 for x in ds if x>0)}/{len(ds)}")
np.save(f"{HERE}/AUDIT_q99q100_ds.npy",np.array(ds))
