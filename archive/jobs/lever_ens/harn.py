...
def init(device="cuda:0"):
    ...
    _R = torch.from_numpy(np.ascontiguousarray(R)).to(DEV)
    _G = torch.from_numpy(np.ascontiguousarray(G)).to(DEV).permute(0, 3, 1, 2).float() / 255.0
...
def build(members, gi, weights=None, round_u8=True):
    idx = torch.as_tensor(resolve(members), device=DEV, dtype=torch.long)
    stack = _R[idx, gi].float()
    if weights is None:
        out = stack.mean(0)
    else:
...
def score_from_per(per, images):
    """recompute mean-score restricted to a subset of image indices"""
