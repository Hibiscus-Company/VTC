#!/bin/bash
# r28 DECIMAL-SAFE = the same energy-restoration operator, but with lambda cut so the zip fits the
# STRICT reading of the cap (350,000,000 bytes) rather than 350 MiB. This is the hedge: if the
# organiser enforces decimal MB, the 340.54 MiB build is rejected outright and a rejected submission
# is worse than a smaller gain. Expected ~+0.14 blended vs ~+0.19 for the MiB build.
...
CAP = 350_000_000                # DECIMAL-SAFE VARIANT: enforce the strict reading
