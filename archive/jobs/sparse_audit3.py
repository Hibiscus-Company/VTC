...
rows=list(csv.DictReader(open(POSES)))[:int(os.environ.get('LIM','9999'))]
...
for r in rows:
    ...
    if not os.path.exists(gp): gp=gp.replace('.jpg','.png'); rp=os.path.join(REN,name)
