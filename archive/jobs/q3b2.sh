...
# dp05 result: overall -1.47, but first8 +1.64 (BEST first8 ever) and last20 -3.71.
# The geometric prior does exactly what the diagnosis predicted on the sparse holes and
# over-constrains the well-observed ones. So the curve needs points BELOW 0.05, not above.
arm long45k_dp01  --iters 45000 --refine_stop 38000 --noise_stop 38000 --lpips_from 30000 --seed 42 --depth_prior 0.01 $DP
arm long45k_dp002 --iters 45000 --refine_stop 38000 --noise_stop 38000 --lpips_from 30000 --seed 42 --depth_prior 0.002 $DP
echo "=== Q3B DONE $(date +%H:%M) ==="; touch $OUT/Q3B2.DONE
