#!/usr/bin/env python
"""SMOOTHING x FIT-DEPTH OVERLAP (all matched amplitude), WITH THE SUBSET-LUCK CONTROL target2.py was missing.
...
ARMS = ["n120m", "n120msm2", "n60am", "n60amsm2", "n60fixm", "n60fixmsm2"]
REF = "n120m"
...
def report(per, acc, N, out, final):
    ...
    if not final:
        return
    print(f"\nSMOOTHING x FIT-DEPTH OVERLAP (all matched amplitude), {TAG}, n={N}, FULL shipped chain, fields in PRODUCTION form "
...
def main():
    ...
    ap.add_argument("--out", default=os.path.join(FR, "res_overlap.json"))
    args = ap.parse_args()
    ...
    gs = lambda f, sg: np.stack([cv2.GaussianBlur(f[..., c], (0, 0), sg) for c in range(2)], -1)
    tgtm = b9mag * 1.30
    ren = lambda f: f * (tgtm / mag(f))
    F = {}
    for a, k in (("n120m", "n120"), ("n60am", "n60a"), ("n60fixm", "n60fix")):
        F[a] = ren(S8[k])
        F[a + "sm2"] = ren(gs(S8[k], 2.0))
