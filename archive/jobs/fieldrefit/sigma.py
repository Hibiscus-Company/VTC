#!/usr/bin/env python
"""FIELD SMOOTHING SWEEP.  shape.py found that gaussian-smoothing the fitted field on its ds=8
...
SIGS = [1.0, 2.0, 3.0, 4.0, 6.0, 10.0]
ARMS = ["ship"] + [f"sm{s:g}" for s in SIGS] + ["sm3raw"]
REF = "ship"
...
def report(per, acc, N, out, final):
    ...
    if not final:
        return
    print(f"\nFIELD SMOOTHING SWEEP, {TAG}, n={N}, FULL shipped chain")
...
def main():
    ...
    ap.add_argument("--out", default=os.path.join(FR, "res_sigma.json"))
    args = ap.parse_args()
